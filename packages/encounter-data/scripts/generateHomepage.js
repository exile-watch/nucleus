  ...
  data.reduce(
    (acc, d) => {
      ...
      switch (d.category) {
        ...
        case "shaper-guardians": {
          return {
            ...
            [d.category]: {
              ...
              thumbnail: !acc[d.category]?.thumbnail
                ? [d.thumbnail].filter(Boolean)
                : acc[d.category]?.thumbnail
                    .concat(d.thumbnail)
                    .filter(Boolean)
                    .slice(0, 4),
        ...
        case "endgame-bosses": {
          const OMITTED_ENDGAME_ENCOUNTERS = [
          ...
          return {
            ...
            main: acc.main?.concat({
              name:
                encounterName === "Fractal Gargantuan"
                  ? "Cortex"
                  : encounterName,
              path,
...
const generateHomepage = async () => {
  ...
  await writeFiles(async ({ data, dir }) => {
    ...
    await fs.writeFileSync(
      `${extractedDataPath}/${dir}/homepage.json`,
      homepage,
    );
  });
