...
const extractSkillTokens = async () => {
  await console.time(
    colorifyConsole({ label: "time", text: "Extract Encounters Spells" }),
  );
  const directories = await getDirectories(tokensPath);

  for await (const dir of directories) {
    const extractedSkills = await yaml.load(
      fs.readFileSync(`${tokensPath}/${dir}/skills.yml`, "utf8"),
    );

    await fs.writeFileSync(
      `${extractedDataPath}/${dir}/skills.json`,
