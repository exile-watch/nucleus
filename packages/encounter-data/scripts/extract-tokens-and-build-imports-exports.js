...
const extractData = async () => {
  ...
  for (const dir of directories) {
    ...
    for (const subDir of subDirectories) {
      await fs.appendFileSync(typesOutputFile, `import { default as ${camelCase(subDir)} } from './extracted-data/${dir}/${subDir}.json'\n`);
      ...
      for (const file of files) {
        ...
        fs.mkdirSync(`${extractedDataPath}/${dir}/${subDir}`, {recursive: true});
        // create json files
        fs.writeFileSync(`${extractedDataPath}/${dir}/${subDir}/${fileName}`, JSON.stringify(data, null, 2));
        // create json imports in encounter-types.ts
        fs.appendFileSync(typesOutputFile, `import { default as ${importName} } from './extracted-data/${dir}/${subDir}/${fileName}'\n`);
      }
    }

    await fs.appendFileSync(typesOutputFile, `import { default as ${camelCase(dir)}IndexedSearch } from './extracted-data/${dir}/indexed-search.json'\n`);
    await fs.appendFileSync(typesOutputFile, `import { default as ${camelCase(dir)}Paths } from './extracted-data/${dir}/paths.json'\n`);
    await fs.appendFileSync(typesOutputFile, `import { default as ${camelCase(dir)}Homepage } from './extracted-data/${dir}/homepage.json'\n`);
    await fs.appendFileSync(typesOutputFile, `import { default as ${camelCase(dir)}Encounters } from './extracted-data/${dir}/encounters.json'\n`);
  }
  ...
  for (const dir of directories) {
    ...
    await fs.appendFileSync(typesOutputFile, `  ${camelCase(dir)}IndexedSearch,\n`);
    ...
    await fs.appendFileSync(typesOutputFile, `  ${camelCase(dir)}Encounters,\n`);
  }
